...
if ! venv_ready; then
  ...
  printf '%s\n' '{"systemMessage": "[memsearch] installing runtime in the background — memory available from the next session"}'
  exit 0
fi
# stdin is inherited untouched: Python reads the payload.
exec "$ROOT/bin/memsearch" hook session-start --platform "$PLATFORM"
