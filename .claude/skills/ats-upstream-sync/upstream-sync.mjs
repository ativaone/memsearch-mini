#!/usr/bin/env node
// Driver for the ats-upstream-sync skill. Dependency-free; reads upstream through `gh api`.
//
...
//   node upstream-sync.mjs record --kind pr --number 606 --verdict applied --reason "..." [--action "..."] [--related 607,608] [--decided-by user]
...
function validate(e) {
  ...
  return {
    ...
    reason: e.reason, action: e.action ?? "", related: (e.related ?? []).map(Number), decided_by: e.decided_by ?? "ai",
  };
}
...
function cmdRecord(flags) {
  ...
  if (flags.has("from-file")) entries = JSON.parse(readFileSync(flags.get("from-file"), "utf8"));
    ...
    kind: flags.get("kind"), number: flags.get("number"), verdict: flags.get("verdict"), reason: flags.get("reason"),
    action: flags.get("action"), related: (flags.get("related") || "").split(",").filter(Boolean), decided_by: flags.get("decided-by"),
    title: flags.get("title"), url: flags.get("url"), upstream_state: flags.get("upstream-state"), upstream_updated_at: flags.get("upstream-updated-at"),
  }];
  ...
  for (const [verdict, list] of groups) {
    ...
    if (compact && archived) {
      ...
      for (const e of list.sort(byNumber)) {
        ...
        console.log(`- ${e.kind}#${e.number} ${e.title}${related}\n    ${what.replace(/\s+/g, " ").trim()}`);
      }
    }
    console.log("");
  }
