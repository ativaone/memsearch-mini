...
You are performing memory retrieval for memsearch-mini. Search past memories and return the most relevant context to the current conversation.

Memory lives in one place per project: `.memsearch-mini/memory/*.md` markdown journals, indexed into a local SQLite database. The CLI runs from the plugin checkout — always call it as `__INSTALL_DIR__/bin/memsearch-mini`, never as a bare `memsearch-mini`.

## Steps

1. **Search**: Run `__INSTALL_DIR__/bin/memsearch-mini search "<query>" -k 5 --json` to find relevant chunks.
...
3. **Expand**: For each relevant result, get the full context:
   - **Primary**: `__INSTALL_DIR__/bin/memsearch-mini expand <chunk_id> --json` returns the full markdown section.
...
4. **Deep drill (optional)**: If an expanded chunk contains a rollout anchor (an HTML comment with session/rollout info) and the original conversation seems critical:
   - Run `__INSTALL_DIR__/bin/memsearch-mini transcript <rollout_path>` to retrieve the original conversation turns, including tool calls.
